...
const STEPS_ONLINE: { status: OrderStatus; label: string; desc: string }[] = [
  { status: "pending_payment", label: "Order placed",       desc: "Verifying your payment…" },
...
const STEPS_OFFLINE: { status: OrderStatus; label: string; desc: string }[] = [
  { status: "pending_payment", label: "Order placed",       desc: "Show your Order ID at the counter to pay" },
  { status: "paid",            label: "Cash received",      desc: "Payment confirmed by the counter" },
  { status: "printing",        label: "Printing",           desc: "Your document is printing now" },
  { status: "completed",       label: "Ready",              desc: "Your printout is ready to collect" },
];
...
export default function OrderTracker({ order }: Props) {
  const STEPS = order.paymentMode === "offline" ? STEPS_OFFLINE : STEPS_ONLINE;
    ...
    <div>
      ...
      <div className="bg-white border border-[#E8E8E8] rounded-xl p-4 mb-6">
        <div className="grid grid-cols-2 gap-4 text-xs">
          ...
          <div>
            <p className="text-[#999] mb-0.5 font-semibold uppercase tracking-wider">Payment</p>
            <p className="font-bold text-[#1A1A1A]">
              {order.paymentMode === "offline" ? "💵 Cash at counter" : "💳 Online / UPI"}
            </p>
          </div>
        </div>
      </div>
