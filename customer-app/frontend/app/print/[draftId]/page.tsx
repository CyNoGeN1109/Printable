...
export default function PrintDraftPage() {
  ...
  const [activePricing, setActivePricing] = useState(storeConfig.pricing);

  // Prefetch checkout so "Proceed to checkout" navigates instantly.
        ...
        try {
          if (d.shopCode) {
            ...
            if (s?.pricing) {
              ...
              setActivePricing({ ...storeConfig.pricing, ...s.pricing });
            }
  ...
  return (
    <main className="min-h-screen bg-[#F2F3F7] text-[#1A1A1A] pb-32">
      ...
      <div className="max-w-lg mx-auto px-5 py-6">
        ...
        {/* No files in draft */}
        {!loading && draft && files.length === 0 && (
          <div className="bg-white border border-[#E8E8E8] rounded-2xl p-8 text-center animate-fadeUp">
            <div className="text-4xl mb-3">📭</div>
            <p className="text-[#D32F2F] text-sm font-bold mb-1">No files found</p>
            <p className="text-[#999] text-xs">Send the file again on WhatsApp to get a fresh link.</p>
          </div>
        )}
